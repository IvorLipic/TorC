...
- Implemented so far: scalar-only `add_scalar`/`sub_scalar`/`mul_scalar`/`div_scalar`/
  ...
  backward (2D + batched with batch-broadcast cases, Step 5), view-op backward
  (`transpose`, `reshape`/`view`, `slice`, Step 6), and `detach()`/`no_grad()` /
  `set_grad_enabled(bool)` (Step 7). Broadcasting reduction (`reduce_sum_to_shape`) is
  applied **centrally** in `Variable::backward_with_grad`, not inside individual backward
  closures — don't duplicate it in a new op's closure
- **Not yet implemented**: in-place-op guarding — see ROADMAP.md's Milestone 4 for exact
  step status
