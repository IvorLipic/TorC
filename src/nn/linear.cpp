...
Linear::Linear(int in_features, int out_features, float init_std, unsigned int seed)
    ...
    if (init_std <= 0.0f) {
        torc::fill_kaiming_normal(weight_data.data(), weight_data.numel(), in_features, seed);
    } else {
